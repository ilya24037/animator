    ...
    <main class="flex-1 min-w-0">
      ...
      <div>
        <ItemCard
          ...
          :item="item"
        />
        <p v-if="activeTab.value === 'pending' && !pending.length" class="text-gray-500">
          Нет объявлений, которые ждут действий
        </p>
        <p v-if="activeTab.value === 'drafts' && !drafts.length" class="text-gray-500">
          У вас пока нет черновиков
        </p>
        <p v-if="activeTab.value === 'archive' && !archive.length" class="text-gray-500">
          Архив пуст
        </p>
      </div>
    </main>
